...
pub fn execute(
    ...
    match msg {
        ExecuteMsg::AddFight{fight_id, fighters, pick} => execute_create_fight(deps, env, info, fight_id, fighters, pick),
    }
...
pub fn query_card_picks(deps: Deps, fight_id: String) -> StdResult<Binary> {
    ...
    match history {
        Err(error) => Err(error),
        Ok(x) => {
            to_binary(&GetCardResponse {picks: x})
        },
    }

    // this binary return is gonna be annoying to deal with on front end as it's 
    // Vec<(String, Pick)>.as_binary
    // 
    // switch it up to make it easier?
}
...
pub fn query_pick(deps: Deps, fight_id: String, fighters: String) -> StdResult<Binary> {
    ...
    match HISTORY.may_load(deps.storage, (fight_id, fighters)) {
        Ok(s) => {
            match s {
                Some(pick) => {
                    to_binary(&GetPickResponse {pick: Some(pick)})
                },
                None => {
                    to_binary(&GetPickResponse {pick: None})
                },
            }
        },
        Err(err) => Err(err)
